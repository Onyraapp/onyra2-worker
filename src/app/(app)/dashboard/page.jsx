...
export default function DashboardPage() {
  ...
  const waClickedRef = useRef(false);
          ...
          <div className="bg-surface rounded-3xl w-full max-w-sm p-6 flex flex-col gap-4 shadow-xl">
            ...
            {!cierreListo ? (
              ...
              <a
                ...
                onClick={(e) => {
                  if (waClickedRef.current) { e.preventDefault(); return; }
                  waClickedRef.current = true;
                  // El cambio visual se aplica después de dejar que el click
                  // navegue primero — si se marca "enviado" en el mismo click
                  // (cambiando el href a undefined en el mismo render), React
                  // puede quitarle el href al link antes de que el navegador
                  // llegue a abrir WhatsApp, y el botón queda gris sin haber
                  // abierto nada.
                  setTimeout(() => setWaEnviado(true), 400);
                }}
                className={`w-full h-12 rounded-xl bg-[#25D366] text-white font-semibold text-[15px] flex items-center justify-center gap-2 active:scale-[0.98] transition-all shadow-sm ${waEnviado ? 'opacity-50' : ''}`}
              >
